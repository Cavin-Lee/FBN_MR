function [x, funVal, ValueL]=LeastRweight(A, y, z, opts)
%
%%
% Function LeastR
...
if (opts.rFlag==0)
    ...
    if (min(z)<0 || max(z)>1)%%*************************************z->min(z),max(z)
...
if (opts.mFlag==0 && opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        ...
        xxp=x-xp;   Axy=Ax-y;
        funVal(iterStep)=Axy'* Axy/2 + rsL2/2 * x'*x + sum(abs(x).*lambda);%%%***********sum(abs(x)) * lambda--->sum(abs(x) * lambda)****************************
