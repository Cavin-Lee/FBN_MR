function [x, funVal, ValueL]=LeastR_TL(A, y, z,meannet, opts)
%
%%
% Function LeastR
...
%%
reweightla=exp(-(sum(abs(meannet+0.00000001).^2)));
...
if (opts.mFlag==0 && opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        ...
        while (1)
            ...
            x=sign(v).*max(abs(v)-lambda * reweightla / L,0);
        ...
        xxp=x-xp;   Axy=Ax-y;
        funVal(iterStep)=Axy'* Axy/2 + rsL2/2 * x'*x + sum(abs(x.*reweightla)) * lambda;
...
if(opts.mFlag==1 && opts.lFlag==0)    
    ...
    for iterStep=1:opts.maxIter
        ...
        xxp=x-xp;   Axy=Ax-y;
        funVal(iterStep)=Axy'* Axy/2 + rsL2/2 * x'*x + sum(t*reweightla) * lambda;
...
if(opts.mFlag==1 && opts.lFlag==1)
    ...
    for iterStep=1:opts.maxIter
        ...
        Axy=Ax-y;
        funVal(iterStep)=Axy' * Axy/2 + rsL2/2 * x'*x + lambda * sum(t*reweightla);
