function [x, funVal, ValueL]=LeastR_HCP(A, y, meannet,gamma,z, opts)
%
%%
% Function LeastR
...
if (opts.mFlag==0 && opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        ...
        g=ATAs-ATy + rsL2 * s+gamma*(x-meannet');%************************** +rsL2*s*inv(sigma)
...
if(opts.mFlag==1 && opts.lFlag==0)    
    ...
    for iterStep=1:opts.maxIter
        ...
        xxp=x-xp;   Axy=Ax-y;
        funVal(iterStep)=Axy'* Axy/2 + rsL2/2 * x'*x + sum(t) * lambda+(x-meannet')'*(x-meannet')*gamma/2;
